...
- `--require-task-completion` is optional. Use it only when the current `.codex-loop/tasks.json` was created for this exact prompt.
...
4. Run `python3 scripts/ralph_session.py start ...` with the parsed task prompt, completion promise, and max iterations.
   - Do not pass `--require-task-completion` by default. Existing task files can be stale leftovers from a previous run.
